...
void MainWindow::changeBlock(QPushButton *button,Block* kamen)
{
	...
	{
		sprintf(nazev,"src/resources/symbol%d.png", kamen->getSymbol());
		QPixmap icon2(nazev);
	...
	if(kamen->isOccupied())
	{
		...
		for(int i = 0; i < 4; i++)
		{
			if(players[i] != NULL)
			{
				sprintf(nazev,"src/resources/player-%d.png", i+1);
				QPixmap icon2(nazev);
...
void MainWindow::changeCard()
{
	...
	if(hraci[hrac] != NULL)
	{
		sprintf(nazev,"src/resources/symbol%d.png", hraci[hrac]->getSymbol());
		QPixmap icon2(nazev);
